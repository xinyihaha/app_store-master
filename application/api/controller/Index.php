...
class Index extends Controller
{
    ...
    public function login($input)
    {
        ...
        if (!$result) {
            ...
            $users = db('users')->where(['username' => $content['username'], 'password' => $content['password']])->find();
			//, 'password' => $password]
            if ($users) {
    ...
    public function config(){
        ...
        $name=$content['username'];
        $result=db('users')->where('Username',$name)->find();
        if($result)//查找到该用户了
        {
    ...
    public function saveConfig()
    {
        ...
        if($result)//查找到该用户了
        {
            ...
            $re=db('users')->where('Username',$name)->update($data);
            if($re)
            {
                header("HTTP/1.1 200 OK");
            }
            ...
            {
                header("HTTP/1.1 404 save failed");
            }
        }
        else
        {
